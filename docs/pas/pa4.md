...
Due date: June 2 23:59 PDT
{: .fs-4 }

[GitHub Classroom Assignment](https://classroom.github.com/a/lyl62d6n){: .btn .btn-blue }
...
{: .note }
There are many shell languages. The most common ones are `bash` and `zsh`.
There is a standard for these shell languages called the [POSIX standard](https://en.wikipedia.org/wiki/POSIX).
...
`pish` will be a simple shell that can run in one of two modes:

### 1. Interactive mode

In interactive mode, your basic shell simply runs on an infinite loop,
where it repeatedly does the following:
...
In script mode, instead of reading from the keyboard, `pish`
...
commands that accomplishes some tasks. You wrote shell scripts
in a prior lab! As you've observed, these scripts can often
...
As you've experienced when using the terminal, users type in commands
to interact with the shell. For this PA, we support two types
of commands: built-in commands and programs.
...
You are not required to write additional unit tests for this PA.
...
[classroom]: https://classroom.github.com/a/lyl62d6n
...
development_. There are many things to do — how should you go about everything?
